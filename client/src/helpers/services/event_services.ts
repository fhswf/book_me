...
export async function updateEvent(
  ...
  const response = await axios.put(
    `${process.env.REACT_APP_API_URL}/events/updateEvent/${id}`,
    {
      data: event
    },
    {
...
export async function getActiveEvents(user_id: string) {
  const response = await axios.get(
    `${process.env.REACT_APP_API_URL}/events/getActiveEvents`,
    {
      params: { user: user_id },
    }
  );
...
export function getAvailableTimes(timeMin: Date, timeMax: Date, url: string, userid: string) {
  return axios.get(
    `${process.env.REACT_APP_API_URL}/events/getAvailable`,
    {
      params: {
        timeMin,
        timeMax,
...
export async function getUsersEvents(token: string) {
  const response = await axios.get(
    `${process.env.REACT_APP_API_URL}/events/getEvents`,
    {
      headers: {
        Authorization: "Bearer " + token,
      },
...
export async function getEventByUrlAndUser(user_id: string, event_url: string) {
  return axios.get(
    `${process.env.REACT_APP_API_URL}/events/getEventBy`,
    {
